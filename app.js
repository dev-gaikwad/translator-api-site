...
function clickHandler(charachterSelect) {
  ...
  fetch(constructURL(inputText, charachterSelect))
    ...
    .catch((error) =>
      alert(
        `Error \nProbably the api call limit reached \nPlease try after sometime`
      )
    );
}

btnTranslateMinion.addEventListener('click', function () {
  clickHandler('minion');
});
btnTranslateYoda.addEventListener('click', function () {
  clickHandler('yoda');
});
btnTranslateKlingon.addEventListener('click', function () {
  clickHandler('klingon');
});
